...
const AddRoom = () => {
  ...
  const onSubmitHandler = async(e)=>{
    ...
    const {data}= await axios.post('/api/roomms/', formData,{headers:{Authorization:`Bearer ${await getToken()}`}})
  ...
  return (
    ...
    <p className='text-gray-800 mt-10'>Images </p>
        ...
        {Object.keys(images).map((key)=>(
          <label key={key} htmlFor={`roomImage${key}`}>
            <img  className='max-h-13 cursor-pointer opacity-80'
             src={images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="uploadArea" />
      ...
      <div className='w-full flex max-sm:flex-cil sm:gap-4 mt-4'>
